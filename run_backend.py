...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(
                project_root / "logs" / "backend.log", encoding="utf-8"
            ),
        ],
    )
...
def start_server(host="0.0.0.0", port=5000, debug=False):
    ...
    try:
        # Run with SocketIO
        socketio.run(
            app, host=host, port=port, debug=debug, use_reloader=debug, log_output=True
        )
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            logger.error(f"💥 Backend crashed: {str(e)}")
