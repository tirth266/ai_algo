...
async def broker_callback(request: Request):
    try:
        ...
        return RedirectResponse(
            f"https://ai-algo-ul1l.vercel.app/broker?status=error&message={str(e)}"
        )
...
@broker_router.post("/connect")
async def broker_connect(request: Request):
    """Connect to broker - placeholder for compatibility"""
    return {"status": "success", "message": "Broker connection endpoint"}
...
async def broker_profile(request: Request):
    try:
        ...
        if kite is None:
            raise HTTPException(
                status_code=401, detail="Session expired, please re-login."
            )
