...
def login_angel_one():
    ...
    if not all([api_key, client_id, totp_seed]):
        raise HTTPException(status_code=500, detail="Missing required environment variables for Angel One")
    ...
    try:
        ...
        if data.get('status'):
            ...
            return {"status": "Success", "message": "Angel One session generated successfully"}
        else:
            raise HTTPException(status_code=400, detail=f"Login failed: {data.get('message', 'Unknown Error')}")
...
def verify_callback(req: VerifyCallbackRequest):
    ...
    return {"status": "Success", "message": "Callback verified and session instantiated"}
