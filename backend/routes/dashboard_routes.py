...
def get_zerodha_dashboard_data() -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # Create Kite instance directly using session data
        try:
        ...
        # Fetch account data
        logger.info("Fetching dashboard data from Zerodha...")
        ...
        # 3. Get orders
        orders = kite.orders()
        today_orders = [o for o in orders if o.get('order_timestamp', '')[:10] == datetime.now().strftime('%Y-%m-%d')]
        ...
        logger.info(f"Dashboard data fetched successfully: Balance={total_margin}, PnL={today_pnl}")
...
def get_cached_dashboard_data() -> Dict[str, Any]:
    ...
    if fresh_data:
        ...
        # Return disconnected state if fetch failed
        return {
            'account_balance': 0,
            'available_margin': 0,
            'used_margin': 0,
