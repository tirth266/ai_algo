...
from ..core.position_manager import PositionManager, create_managed_signal
    ...
    def generate_signal(self, candles: pd.DataFrame) -> Optional[str]:
        ...
        if bullish_flip and ema_alignment == "bullish" and structure == "hh_hl":
            if volume > volume_ma and current_close > ema50:
                ...
                self.stop_loss = current_close - (
                    self.atr_sl_multiplier * df["atr"].iloc[-1]
                )
                self.take_profit = current_close + (
                    self.risk_reward_ratio * self.atr_sl_multiplier * df["atr"].iloc[-1]
                )

                managed = create_managed_signal(
                    candles,
                    "BUY",
                    self.entry_price,
                    self.stop_loss,
                ...
                self.position_config = managed
                logger.info(
                    f"BUY SIGNAL: Trend flip + EMA bullish + HH/HL + volume confirmation"
                )
                return "BUY"
        ...
        if bearish_flip and ema_alignment == "bearish" and structure == "lh_ll":
            if volume > volume_ma and current_close < ema50:
                ...
                self.position_config = managed
                logger.info(
                    f"SELL SIGNAL: Trend flip + EMA bearish + LH/LL + volume confirmation"
                )
                return "SELL"
...
class Backtester:
    ...
    def _close_all(self, candle: pd.Series):
        for position in self.positions:
            self._execute_exit(
                position, candle["close"], "END_OF_BACKTEST", candle.name
            )
